...
void handle_clients(int client_sockets[], int total_students, int monitor_socket) {
  ...
  send(monitor_socket, results, sizeof(results), 0);
...
int main(int argc, char *argv[]) {
  ...
  int monitor_socket;
  if ((monitor_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t *)&addrlen)) < 0) {
    perror("accept");
    exit(EXIT_FAILURE);
  }
  ...
  srand(time(NULL));
  handle_clients(client_sockets, total_students, monitor_socket);
